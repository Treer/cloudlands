...
local VINES_REQUIRED_HUMIDITY    = 50
local VINES_REQUIRED_TEMPERATURE = 30
...
local coreTypes = {
  ...
  },
  {
    ...
    thicknessMax      = 2,
    frequency         = 0.1,
    pondWallBuffer    = 0.11, -- larger values will make ponds smaller and further from island edges, so it should be as low as you can get it without the ponds leaking over the edge. 0.154 is required for a small island at (3160, -2360) on seed 1
    requiresNexus     = false,
    exclusive         = true
  }
}
...
local function addDetail_vines(decoration_list, core, data, area, minp, maxp)
  ...
  if VINE_COVERAGE > 0 and nodeId_vine ~= nodeId_ignore then
    ...
    if y >= minp.y and y <= maxp.y then      
      ...
      if core.biome ~= nil and core.biome.humidity_point >= VINES_REQUIRED_HUMIDITY and core.biome.heat_point >= VINES_REQUIRED_TEMPERATURE then
...
local function addDetail_skyReef(decoration_list, core, data, area, minp, maxp)
  ...
  local nodeId_top
  local nodeId_filler
  if core.biome == nil then -- We can't assume the core biome has already been resolved, core might not have been big enough to enter the draw region
    core.biomeId = minetest.get_biome_data({x = coreX, y = coreTop, z = coreZ}).biome
      ...
      for _,core in pairs(cores) do
        ...
        if distanceSquared <= radiusSquared then
          ...
          if core.biome == nil then
            core.biomeId = minetest.get_biome_data({x = core.x, y = coreTop, z = core.z}).biome
            core.biome = biomes[core.biomeId]
          end
          ...
          if surfaceNoise < 0 and densityNoise ~= nil then
